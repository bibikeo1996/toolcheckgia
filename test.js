...
function expxortLink(data){
    ...
    var today1 = new Date();
    var filename1 = "LinkDMCL" +'.xlsx';
    workbook.write("./public/"+filename1);
}
...
function exportToExcelDoiThu(data) {
    ...
    var today = new Date();
    var filename = "GiaDMX-NK"  + '.xlsx'; //+ Date.now().toString()
...
function exportToExcel(data) {
    ...
    var filename = "GiaDMCL"  + '.xlsx'; //+ Date.now().toString()
    workbook.write("./public/"+filename);
}
...
app.post('/checkgiadmcl',upload.single('checkgiadmcl'),function(req,res){
        	...
        	(async () => {
			    ...
			    expxortLink(result);
			    res.write("<a href='/LinkDMCL.xlsx'>Download File Link</a>");
			    for(var rs2 of result){ // open for check giá 
			    	try{
			    		...
			    		const info = await page.evaluate(() => { // open page evaluate 
						 ...
						 if(info){
			            	arrInfo.push(info)
			                res.write("<p>" + rs2.STT + " " + info.Sap + " => Xong!</p>")
			            }
			    	} catch (err) {
			    ...
			    res.end("<a href='/GiaDMCL.xlsx'>Download File</a>");
    		...
    		(async () => { // open async 2
		    	...
		    	for(var rs of urls){ // open for 2
					...
					if(info){
			                ...
			                res.write("<p>Số thứ tự" + rs.STT + " Mã Article " + info.Sap + " => Xong!</p>")
			            }
		    		} catch (err){
		    			 res.write("<p>Lỗi => " + rs.STT +"</p>")
		    		}
		    	} // end for 2
		    	exportToExcel(arrInfo);
			    res.write("<a href='/'>Back</a></br>");
...
app.post('/checkgiankdmx',upload.single('checkgiankdmx'),function(req,res){
    ...
    res.write("<a href='/'>Back</a></br>");
	res.end("<a href='/GiaDMX-NK.xlsx'>Download File</a>");
	})();
});
